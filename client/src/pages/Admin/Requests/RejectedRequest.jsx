  ...
  const navigation = useNavigate();
    ...
    useEffect(() => {
      const fetchLockers = async () => {
        try {
          const response = await axios.get(`${API_URL}/api/v1/request/rejected-request`);
    ...
    const scrollableStyle = {
      ...
      const handlePending = async (id) => {
        try {
          const response = await axios.put(`${API_URL}/api/v1/request/pending/${id}`);
          if(response){
            toast.success('Request Pending Successfully');
            navigation("/requests/pending")
          }
          // setUsers(users.filter((user) => user.id !== id));
          } catch (error) {
            console.error("Error Pending user:", error);
            ...
            <div className="col-md-10">
                ...
                <table className="table table-striped table-hover">
                    ...
                    {users.length > 0 ? (
                        ...
                        <td><NavLink className="btn btn-sm btn-danger" id="update" onClick={() => handlePending(user._id)} >Pending</NavLink></td>
                    </tr>
                ))
                                 ) : (
                                   <tr>
